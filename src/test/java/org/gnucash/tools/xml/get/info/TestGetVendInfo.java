...
{
  // Logger
  private static Logger logger = Logger.getLogger(TestGetVendInfo.class);
  ...
  public static void main( String[] args )
  {
    try
    {
      TestGetVendInfo tool = new TestGetVendInfo ();
      tool.execute(args);
    }
...
//    getConfigSettings(cfg);
  ...
  protected void kernel() throws Exception
  {
    ...
    else if ( mode == Helper.Mode.NAME )
    {
      ...
      vend = new GnuCashWritableVendorImpl( (GnuCashVendorImpl) vendList.iterator().next() );
    }
    ...
    try
    {
      System.out.println("toString:          " + vend.toString());
    }
    catch ( Exception exc )
    {
    ...
    try
    {
      System.out.println("Terms:             " + vend.getTerms());
    }
    catch ( Exception exc )
    {
    ...
    if ( showInvoices )
      showInvoices((GnuCashWritableVendorImpl) vend);
  }
  ...
  private void showInvoices(GnuCashWritableVendorImpl vend) throws Exception
  {
    System.out.println("");
    System.out.println("Invoices:");

    System.out.println("Number of open invoices: " + vend.getNofOpenBills());

    System.out.println("");
    System.out.println("Paid invoices (direct):");
    for ( GnuCashWritableVendorBill invc : vend.getPaidWritableBills_direct() )
    {
...
//    for ( GnuCashJobInvoice invc : vend.getPaidWritableInvoices_viaAllJobs() )
//    {
//      System.out.println(" - " + invc.toString());
//    }
    ...
    for ( GnuCashWritableVendorBill invc : vend.getUnpaidWritableBills_direct() )
    {
      System.out.println(" - " + invc.toString());
    }
...
//    for ( GnuCashJobInvoice invc : vend.getUnpaidWritableInvoices_viaAllJobs() )
//    {
//      System.out.println(" - " + invc.toString());
//    }
  }
  ...
  protected void printUsage()
  {
    ...
    formatter.printHelp( "TestGetVendInfo", options );
